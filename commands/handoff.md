...
1. Availability check; resolve identity; locate the existing record at `record_path` (update it, do not duplicate).
...
4. Redact via `scripts/redact.py`; `update_document`; refresh `Last updated:`; set the `Tasks` parent flag (`handoff` maps to `doing`, `blocked` to `blocked`).
